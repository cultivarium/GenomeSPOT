...
from helpers import gtdb_taxonomy_to_tuple, gtdb_accession_to_ncbi
...
class PhylogeneticPartition():
    ...
    def __init__(self, 
        ...
        self.taxonomy_dict = self.make_taxonomy_dict(filename=taxonomy_file, source=taxonomy_format)
    ...
    def select_genomes_at_rank(self, 
                               genomes : list,  
                               rank : str, 
                               n_genomes : int=1,
                               ):
    ...
    def balance_dataset(self, subset_genomes : list, proportion_to_keep : float=None, diversity_rank : str='species'):
        """
        ...
        for rank, i in self.taxonomy_indices.items():
            ...
            n_observed = self.measure_diversity(rank, diversity_rank, subset_genomes=subset_genomes)
        ...
        balanced_genomes = rng.choice(subset_genomes, 
                                    n_selections, 
    ...
    def partition(self, subset_genomes : set, partition_size : float) -> set:
        ...
        iterator_counts = self.measure_diversity(self.iteration_rank, self.diversity_rank, subset_genomes=subset_genomes)
        ...
        random_iterator_order = rng.choice(iterator_list, 
                                        n_partition_taxa, 
                                        p=iterator_freqs, 
                                        replace=True
                                        )
    ...
    def balanced_partition(self, 
        ...
        if dereplicate_species:
            ...
            representative_genomes = self.select_genomes_at_rank(genomes=self.genomes,
                                                                n_genomes = 1,
                                                                rank='species',
                                                                )
        else:
        ...
        balanced_genomes = self.balance_dataset(dereplicate_species=dereplicate_species, 
    ...
    def assess_distance_to_reference(self, subset_genomes, rank : str='phylum'):
        ...
        n_selected = self.measure_diversity(rank, 'species', subset_genomes=subset_genomes)
    ...
    def assess_proportion(self, subset_genomes, reference_genomes, rank : str='phylum'):
        ...
        n_selected = self.measure_diversity(rank, 'species', subset_genomes=subset_genomes)
        n_reference = self.measure_diversity(rank, 'species', subset_genomes=reference_genomes)
